...
- `.env`: committed public defaults for env vars.
- `.env.local.example`: committed template for secrets; users copy to `.env.local` (gitignored).
...
| Tracing wiring or new backend | README "Tracing" table, CLAUDE.md gotchas, ADR 0004, `.env` (public) or `.env.local.example` (secrets) |
...
| New env var (public default) | `.env`, README quick start, CLAUDE.md if it has an associated gotcha |
| New env var (secret) | `.env.local.example`, README "Fill in your secret keys", CLAUDE.md if it has an associated gotcha |
